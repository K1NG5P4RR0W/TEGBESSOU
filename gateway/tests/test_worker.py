"""Tests directs du worker (E1 + E3b) : pas de DB, pas de réseau réel — un
script factice tient lieu de binaire subfinder pour prouver que
`run_subfinder` suit le même modèle (timeout/kill) que `run_noop`, sans
dépendre du binaire réel ni d'une sortie réseau (voir README pour la preuve
réseau réelle, faite en runtime Docker)."""
...
from app.worker import run_noop, run_subfinder
...
def _fake_subfinder_script(tmp_path, stdout: str, sleep_seconds: float = 0.0) -> list[str]:
...
async def test_run_subfinder_success(tmp_path) -> None:
    ...
    args = _fake_subfinder_script(tmp_path, '{"host":"api.example.com"}\n')
...
async def test_run_subfinder_timeout_is_failed(tmp_path) -> None:
    ...
    args = _fake_subfinder_script(tmp_path, "", sleep_seconds=5.0)
